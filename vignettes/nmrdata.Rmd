...
In addition to the processed dataset (`bariatric`), the package also contains a minimal set 
of 21 raw Bruker NMR experiment folders, located under `inst/extdata/`. 
These files can be accessed using the `system.file()` function, e.g.:
...
Both, processed data and raw experiment files are primarily intended for demonstrating 
functionality of the  companion software package `metabom8`.
...
- `meta`: Acquisition and processing status parameters (Topspin), prefixed with `a` and `p`, respectively
